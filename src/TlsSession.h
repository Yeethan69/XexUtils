...
struct TlsSessionContext
{
    br_ssl_client_context SslClientContext;
    br_x509_minimal_context x509Context;
    br_sslio_context IoContext;           
    uint8_t IoBuffer[BR_SSL_BUFSIZE_BIDI];               
    br_x509_trust_anchor TrustAnchors[MAX_ANCHORS];
    size_t TrustAnchorCount; 
};
...
/// @brief Initialises a TlsSessionContext and child structs to all 0s ready for use.
///
/// Must be called before a TlsSessionContext gets used.
///
/// @param context The TlsSessionContext to act upon.
bool InitContext(TlsSessionContext*& context);

/// @brief Frees a TlsSessionContext for cleanup.
///
/// @param context The TlsSessionContext to act upon.
...
void Start(TlsSessionContext* context, const SOCKET &socket, const std::string &domain);
